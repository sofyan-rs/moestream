    ...
    return (
        <View className="p-5 flex-col gap-3">
            ...
            */}
            ...
            {/* Prev / Next */}
            <View className='flex-row gap-2.5'>
                <Button variant="outline" className={cn('flex-1 bg-surface', hasPrev ? 'border-accent' : '')} isDisabled={!hasPrev}
                    onPress={onPrev}>
                    <SkipPrevious size={16} color={hasPrev ? accentColor : iconColor} />
                    <Text className={cn('font-semibold text-sm', hasPrev ? 'text-accent' : 'text-muted')}>
                        Previous
                    </Text>
                </Button>
                <Button variant="outline" className={cn('flex-1 bg-surface', hasNext ? 'border-accent' : '')} isDisabled={!hasNext}
                    onPress={onNext}>
                    <Text className={cn('font-semibold text-sm', hasNext ? 'text-accent' : 'text-muted')}>
                        Next
                    </Text>
